...
const SYS = `Output ONLY raw JSON object. Format: {"code":"<!DOCTYPE html>...</html>","explanation":"magyar szöveg"}. No markdown. No backticks. Nothing outside the JSON.`;
...
async function gen(cmd: string, existing: string) {
  ...
  const t: string = (d.content || []).map((b: any) => b.text || "").join("");
  const start = t.indexOf("{");
  const end = t.lastIndexOf("}");
  if (start === -1 || end === -1) throw new Error("API hiba: " + t.slice(0, 150));
  const obj = JSON.parse(t.slice(start, end + 1));
  if (!obj.code) throw new Error("Nincs code mező: " + t.slice(0, 150));
  return obj;
}
  ...
  return (
    <div style={{fontFamily:"monospace",background:"#080808",minHeight:"100vh",color:"#d0d0d0",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"14px 20px",borderBottom:"1px solid #181818",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,background:"#080808",zIndex:10}}>
        ...
        <span style={{fontSize:9,color:ac}}>{{idle:"KÉSZEN",thinking:"GENERÁL...",done:"KÉSZ",error:"HIBA"}[status]}</span>
      </div>
      <div style={{display:"flex",borderBottom:"1px solid #181818"}}>
        {(["input","preview"] as const).map(id=>(
        ...
        <div style={{flex:1,padding:20,display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{border:`1px solid ${status==="thinking"?"#00cfff44":"#222"}`,borderRadius:6,overflow:"hidden"}}>
            <textarea value={input} onChange={e=>setInput(e.target.value)} rows={4}
              placeholder={code?'Módosítsd...':'Írd le az appot...'}
